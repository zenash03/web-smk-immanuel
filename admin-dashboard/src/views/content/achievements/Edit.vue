<template>
    <div>
        <form action="" @submit.prevent="submitAchievements" method="post" enctype="multipart/form-data" id="form">
...
export default {
    ...
    methods: {
        submitAchievements() {
            ...
            axios.post(`admin/achievements/${this.id}?token=${this.token}`, formData)
                .then(res => {
                    this.$router.push('/content/achievements');
                })
                .catch(err => {
        ...
        getData() {
            axios.get(`achievements/${this.id}?token=${this.token}`)
                .then(res => {
                    this.data = res.data;
