<template>
    <div>
        ...
        <h3 class="mb-5 w-30">Filter Data</h3>

        <div class="row" v-if="!custom">
            <div class="col-lg-4 mb-4" v-for="filter in filters" :key="filter.name">
                <div class="card">
                    <div class="card-header pt-3 pb-3">
                        <h5 class="title m-0">{{ filter.name }}</h5>
                    </div>
                    <div class="card-body p-2">
                        <p class="m-0">Semua Siswa: <b class="text-info">{{ filter.jumlah_siswa }}</b></p>
                        <p class="m-0">Sudah Mendaftar: <b class="text-success">{{ filter.jumlah_pendaftar }}</b></p>
                        <p class="mb-4">Belum Mendaftar: <b class="text-danger">{{ filter.jumlah_siswa - filter.jumlah_pendaftar }}</b></p>

                        <button type="button" class="form-control btn-primary mb-2" @click="showFilter(filter.name)">Lihat</button>
                    </div>
                </div>
            </div>
        </div>

        <div class="table-container mb-5" v-if="data">
            <h5 class="text-success">Sudah Mendaftar</h5>
            <table class="table">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                ...
                <tbody>
                    <tr v-for="(d, i) in data.registered" :key="d.id">
                        <th scope="row">{{ i + 1 }}</th>
                        <td>{{ d.nama }}</td>
                        <td>{{ d.username }}</td>
                        <td>{{ d.kelas }}</td>
        ...
        <div class="table-container" v-if="data">
            <h5 class="text-danger">Belum Mendaftar</h5>
            <table class="table">
                <thead>
                    <tr>
                        <th scope="col">#</th>
                ...
                <tbody>
                    <tr v-for="(d, i) in data.unregistered" :key="d.id">
                        <th scope="row">{{ i + 1 }}</th>
                        <td>{{ d.name }}</td>
                        <td>{{ d.username }}</td>
                        <td>{{ d.kelas }}</td>
        ...
        <button type="button" class="form-control btn-info mt-5" v-if="custom" @click="custom = null, data = null">Clear Filter</button>
    </div>
</template>
...
export default {
    ...
    methods: {
        getFilters() {
            axios.get(`admin/magang/filter?token=${this.token}`)
                .then(res => {
        ...
        getData() {
            axios.get(`admin/magang/filter/key?key=${this.custom}&token=${this.token}`)
                .then(res => {
                    this.data = res.data;
                })
                .catch(err => {
